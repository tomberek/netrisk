...
module Netrisk.Core (totalRisk,flowParser) where
...
-- | Selector: picks the source or destination.
-- Bypass: Just will continue the flow. const Nothing will short circuit.
-- Processor: Nothing stops the flow. Just continues it.
-- Table: the IPRTable
ruleProcessor :: (Netflow -> IPv4) -> (FlowType -> Maybe FlowType) -> (FlowType -> output -> Maybe FlowType) -> RuleFlow output
...
-- instance Eq Netflow where (==) = (==) `on` pair
-- instance Ord Netflow where compare = compare `on` pair
instance Hashable Netflow
instance Hashable IPv4
instance Hashable FlowType
-- | The main entry point. Duplicated data to obtain 1 million rows. Executes in ~ .25s on 8 cores.
main :: IO ()
main = do
    (file:_) <- getArgs
